    ...
    return (
      <Layout>
        <div className="text-center py-20">
          ...
          <Link to="/lernen" className="text-sm mt-2 inline-block" style={{ color: 'var(--accent)' }}>← Zurück</Link>
        </div>
      </Layout>
    );
  }
    ...
    <Layout>
      <div className="max-w-xl mx-auto">
          ...
          {(progress.bestQuizScore > 0 || progress.bestTestScore > 0) && (
            <div className="flex gap-4 mt-3">
              {progress.bestQuizScore > 0 && (
                <span className="text-xs px-2 py-1 rounded-full border border-[#ebebeb] dark:border-[#2a2a2a]" style={{ color: 'var(--accent)' }}>
                  Quiz: {progress.bestQuizScore}%
                </span>
              )}
              {progress.bestTestScore > 0 && (
                <span className="text-xs px-2 py-1 rounded-full border border-[#ebebeb] dark:border-[#2a2a2a]" style={{ color: '#1D9E75' }}>
        ...
        {weakCardIds.length > 0 && (
          <div className="rounded-xl p-4 mb-6 flex items-center justify-between gap-3 border bg-card app-border">
            <div>
            ...
            <button
              ...
              className="shrink-0 text-sm font-medium px-4 py-2 rounded-lg transition-opacity hover:opacity-80 cursor-pointer bg-[#111111] dark:bg-white text-white dark:text-[#111111]"
            >
              Jetzt üben
            </button>
          </div>
        ...
        >
          ...
          <div className="relative w-10 h-6 rounded-full shrink-0 transition-colors" style={{ backgroundColor: mixed ? 'var(--accent)' : '#ebebeb' }}>
        ...
        <div className="space-y-3">
          {MODES.map((mode) => {
            ...
            return (
              ...
              >
                <mode.icon size={24} className="shrink-0" style={{ color: 'var(--accent)' }} />
